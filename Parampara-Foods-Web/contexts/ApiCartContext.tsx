...
import { apiClient, FoodDto, CategoryDto, AuthResponse, OrderCreateDto, OrderDto, WishlistDto, GoogleAuthDto, GoogleAuthResponse, PhoneAuthResponse, PhoneVerificationResponse } from '../services/apiClient';
...
export const ApiCartProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (path && path !== '') {
      ...
      const validPages = ['home', 'cart', 'login', 'register', 'category', 'productDetail', 'checkout', 'success', 'about', 'contact', 'searchResults', 'wishlist', 'myOrders', 'adminDashboard', 'adminProducts', 'adminUsers', 'adminRoles', 'adminOrders', 'adminAnalytics', 'adminReviews', 'adminCategories', 'account'];
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    try {
      ...
      // Load user's wishlist and orders after successful login
      await loadWishlist();
  ...
  const logout = () => {
    ...
    setUserOrders([]); // Clear user orders on logout
  };
  ...
  const placeOrder = async (deliveryAddress: string, customerNotes?: string): Promise<boolean> => {
    ...
    try {
      ...
      clearCart();
      // Reload user orders after placing a new order
      await loadUserOrders();
      setPage('success');
      return true;
    } catch (err) {
